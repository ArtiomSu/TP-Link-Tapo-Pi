...
export default class P100 {
    ...
    constructor(
      ...
      //console.log('Constructing P100 on host: ' + ipAddress);
      this.ip = ipAddress;
    ...
    async handshake():Promise<boolean>{
      ...
      //console.log('Handshake P100 on host: ' + this.ip);
      ...
      try{
        const res:AxiosResponse = await this.axios.post(URL, payload, config);
        //console.log('Received Handshake P100 on host response: ' + this.ip);

        if(res.data.error_code){
          if(res.data.error_code === 1003){
            // not really error so don't log
          }else{
            console.error("handshake error", res.data);
          }
        ...
        try{
          ...
          const cooc = res.headers['set-cookie'];
          if(cooc){
            this.cookie = cooc[0].split(';')[0];
          }else{
            throw Error('no set-cookie header found');
          }
          return true;
        } catch (error){
          throw this.handleError(res.data.error_code, '106');
        }
      }catch(error: any){
        //console.error('111 Error: ' + error.message);
    ...
    async login():Promise<boolean>{
      ...
      if(this.tpLinkCipher){
        ...
        try{
          const res:AxiosResponse = await this.axios.post(URL, securePassthroughPayload, config);
          if(res.data.error_code){
            throw this.handleError(res.data.error_code, '146');
          }
          const decryptedResponse = this.tpLinkCipher.decrypt(res.data.result.response);
          try{
            const response = JSON.parse(decryptedResponse);
            if(response.error_code !== 0){
              throw this.handleError(res.data.error_code, '152');
            }
            this.token = response.result.token;
            return true;
          } catch (error){
            throw this.handleError(JSON.parse(decryptedResponse).error_code, '157');
          }
        }catch(error: any) {
        ...
        throw new Error('no tpLinkCipher');
      }
    }
    ...
    async raw_request(path: string, data: Buffer, responseType: string, params?:any):Promise<any>{
      ...
      return this.axios.post(URL, data, config)
        ...
        .catch((error: Error) => {
          console.error('322 Error: ' + error.message);
          return error;
        });
    }
    ...
    private decode_handshake_key(key:string){
      ...
      this.tpLinkCipher = new TpLinkCipher(b_arr, b_arr2);
    }
    ...
    async getDeviceInfo(): Promise<PlugSysinfo>{
      if(this.getSysInfo() && (Date.now() - this.getSysInfo().last_update) < this._timeout){
      ...
      if(this.tpLinkCipher){
        ...
        try{
          const res = await this.axios.post(URL, securePassthroughPayload, config);

          if(res.data.error_code){
            if((res.data.error_code === '9999' || res.data.error_code === 9999) && this._reconnect_counter <= 3){
              console.error(' Error Code: ' + res.data.error_code + ', ' + this.ERROR_CODES[res.data.error_code]);
            ...
            this._reconnect_counter = 0;
            throw this.handleError(res.data.error_code, '326');
          }
          ...
          try{
            const response = JSON.parse(decryptedResponse);
            if(response.error_code !== 0){
              throw this.handleError(response.error_code, '333');
            }
            ...
            throw this.handleError(JSON.parse(decryptedResponse).error_code, '340');
          }
        }catch(error : any){
          console.log('371 Error: ' + error.message);
          throw error;
        };
   ...
   */
    ...
    protected handleError(errorCode:string | number, line:string):boolean{
      const errorMessage = this.ERROR_CODES[errorCode];
      console.error(line + ' Error Code: ' + errorCode + ', ' + errorMessage + ' ' + this.ip);
